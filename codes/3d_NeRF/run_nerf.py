...
def run_network(inputs, viewdirs, fn, embed_fn, embeddirs_fn, netchunk=1024*64,
    ...
    if viewdirs is not None:
        ...
        if current_iter is not None and reg_end_iter is not None:
            view_freq_mask = get_freq_mask(embedded_dirs.shape, current_iter, reg_end_iter) #Carry out frequency masking (regularization)
...
def batchifyadd(fn, chunk):
    """Constructs a version of 'fn' that applies to smaller batches.
    """
    if chunk is None:
        return fn
    def ret(inputs, residual):
        return torch.cat([fn([inputs[i:i+chunk],residual[i:i+chunk]]) for i in range(0, inputs.shape[0], chunk)], 0)
    return ret

def run_network_add(inputs, viewdirs, fn, embed_fn, embed_fn_fake, embeddirs_fn, embeddirs_fn_fake, netchunk=1024*64,
                current_iter=None, reg_end_iter=None):
    """Prepares inputs and applies network 'fn'.
    """
    inputs_flat = torch.reshape(inputs, [-1, inputs.shape[-1]])
    embedded = embed_fn(inputs_flat) # fake embeding as flat input
    residual = embed_fn_fake(inputs_flat) # postional encoding as residual
    coord_freq_mask = torch.ones(1).cuda().float()
    if current_iter is not None and reg_end_iter is not None:
        coord_freq_mask = get_freq_mask(embedded.shape, current_iter, reg_end_iter)
        embedded = embedded * coord_freq_mask
        residual = residual * coord_freq_mask

    if viewdirs is not None:
        input_dirs = viewdirs[:,None].expand(inputs.shape)
        input_dirs_flat = torch.reshape(input_dirs, [-1, input_dirs.shape[-1]])
        embedded_dirs = embeddirs_fn(input_dirs_flat)
        residual_dirs = embeddirs_fn_fake(input_dirs_flat)
        if current_iter is not None and reg_end_iter is not None:
            view_freq_mask = get_freq_mask(embedded_dirs.shape, current_iter, reg_end_iter) #Carry out frequency masking (regularization)
            embedded_dirs = embedded_dirs * view_freq_mask
            residual_dirs = residual_dirs * view_freq_mask
        embedded = torch.cat([embedded, embedded_dirs], -1)
        residual = torch.cat([residual, residual_dirs], -1)

    outputs_flat = batchifyadd(fn, netchunk)(embedded, residual)
    outputs = torch.reshape(outputs_flat, list(inputs.shape[:-1]) + [outputs_flat.shape[-1]])
    return outputs, 1 - coord_freq_mask.mean().item()
...
def create_nerf(args):
    ...
    disable_pos_enc = False
    use_res_encoder = True
    embed_fn, input_ch = get_embedder(args.multires, args.i_embed, disable_pos_enc = disable_pos_enc)
    if use_res_encoder:
        embed_fn_fake, input_ch_fake = get_embedder(args.multires, args.i_embed, disable_pos_enc = True)

    input_ch_views = 0
    embeddirs_fn = None
    if args.use_viewdirs:
        embeddirs_fn, input_ch_views = get_embedder(args.multires_views, args.i_embed, disable_pos_enc = disable_pos_enc)
        if use_res_encoder:
            embeddirs_fn_fake, input_ch_views_fake = get_embedder(args.multires_views, args.i_embed, disable_pos_enc = True)
    output_ch = 5 if args.N_importance > 0 else 4
    skips = [4]
    if use_res_encoder:
        # model = FineNeRFAddDir(D=args.netdepth, W=args.netwidth,
        #         input_ch=input_ch, output_ch=output_ch, skips=skips,
        #         input_ch_views=input_ch_views, use_viewdirs=args.use_viewdirs)
        model = FineNeRFAdd(D=args.netdepth, W=args.netwidth,
                input_ch=input_ch, output_ch=output_ch, skips=skips,
                input_ch_views=input_ch_views, use_viewdirs=args.use_viewdirs)
    else:
        model = FineNeRF(D=args.netdepth, W=args.netwidth,
                    input_ch=input_ch, output_ch=output_ch, skips=skips,
                    input_ch_views=input_ch_views, use_viewdirs=args.use_viewdirs)
        # model = SineNeRF(D=args.netdepth, W=args.netwidth,
        #              input_ch=input_ch, output_ch=output_ch, skips=skips,
        #              input_ch_views=input_ch_views, use_viewdirs=args.use_viewdirs)
    ...
    model_fine = None #fine model for RGB and coarse model for density
    if args.N_importance > 0:
        if use_res_encoder:
            # model_fine = FineNeRFAddDir(D=args.netdepth_fine, W=args.netwidth_fine,
            #               input_ch=input_ch, output_ch=output_ch, skips=skips,
            #               input_ch_views=input_ch_views, use_viewdirs=args.use_viewdirs)
            model_fine = FineNeRFAdd(D=args.netdepth_fine, W=args.netwidth_fine,
                          input_ch=input_ch, output_ch=output_ch, skips=skips,
                          input_ch_views=input_ch_views, use_viewdirs=args.use_viewdirs)
        else:
            model_fine = FineNeRF(D=args.netdepth_fine, W=args.netwidth_fine,
                            input_ch=input_ch, output_ch=output_ch, skips=skips,
                            input_ch_views=input_ch_views, use_viewdirs=args.use_viewdirs)
            # model_fine = SineNeRF(D=args.netdepth_fine, W=args.netwidth_fine,
            #                   input_ch=input_ch, output_ch=output_ch, skips=skips,
            #                   input_ch_views=input_ch_views, use_viewdirs=args.use_viewdirs)
    ...
    if use_res_encoder:
        network_query_fn = lambda inputs, viewdirs, network_fn, current_iter, reg_end_iter : run_network_add(inputs, viewdirs, network_fn,
                                                                    embed_fn=embed_fn,
                                                                    embed_fn_fake=embed_fn_fake,
                                                                    embeddirs_fn=embeddirs_fn,
                                                                    embeddirs_fn_fake=embeddirs_fn_fake,
...
def create_dnet(args, critic = False, dnettype='Conv1D'):
    if dnettype == 'CNN':
        if critic:
            dnet = CriticCNN()
        else:
            dnet = DiscriminatorCNN()
        optimizerD = torch.optim.Adam(dnet.parameters(), lr=args.lrate, betas=(0.9, 0.999))
    elif dnettype == 'Conv1D':
        if critic:
            dnet = CriticConv1D()
        else:
            dnet = DiscriminatorConv1D()
        optimizerD = torch.optim.Adam(dnet.parameters(), lr=args.lrate, betas=(0.9, 0.999))
    return dnet, optimizerD
...
def sample_rays(H, W, rays_o, rays_d, N_rand, i, start, precrop_iters, precrop_frac, randomsampling=True):
    if i < precrop_iters:
    ...
    # random choose N_rand pixels does not perform low pass filtering, but introduce high frequency noise
    if randomsampling:
        select_inds = np.random.choice(coords.shape[0], size=[N_rand], replace=False)  # (N_rand,)
    else:
        segment_portion = 0.3
        start_ind = np.random.randint(0, coords.shape[0])#((i * N_rand) % coords.shape[0])
        random_inject = np.random.randint(0, int(N_rand*(1-segment_portion)))# Inject random segment in the middle
        end_ind = start_ind + int(N_rand*segment_portion) if start_ind + int(N_rand*segment_portion) < coords.shape[0] else coords.shape[0]
        if end_ind - start_ind < N_rand: # concatenate random segments and random samples
            #select_inds = np.concatenate([np.arange(start_ind, end_ind), np.random.choice(coords.shape[0], size=[N_rand - (end_ind - start_ind)], replace=False)])
            select_inds = np.random.choice(coords.shape[0], size=[N_rand], replace=False)
            select_inds[random_inject:random_inject+end_ind-start_ind] = np.arange(start_ind, end_ind)
        else:
            select_inds = np.random.choice(coords.shape[0], size=[N_rand], replace=False) 
...
def train():
    ...
    USE_GAN = True
    WGAN = False
    USE_RANDOM_RAYS = False

    if USE_GAN:
        dnet, optimizer_d = create_dnet(args, critic=WGAN)
        dnet = dnet.to(device)
    ...
    print('Use render loss?', any_rendered_loss)
    ...
    for i in trange(start, N_iters):
        ...
        if use_batching:
            ...
            for target, rays_o, rays_d in zip(images[i_train], rays_rgb_o, rays_rgb_d):
                rays_pose, select_coords = sample_rays(H, W, rays_o, rays_d, N_rand=N_rand_per_view,
                    i=i, start=start, precrop_iters=args.precrop_iters, precrop_frac=args.precrop_frac, randomsampling=USE_RANDOM_RAYS)
        ...
        if calc_ctr_loss and render_loss_iter:
            ...
            print('Embed Shape', rendered_embedding.shape)
            # ffl_loss = ffl(target_emb, rendered_emb0)
            # loss = loss + ffl_loss
        ...
        if not args.no_mse:
            ...
            #print(rgb.shape, target_s.shape) #[1024,3]
            #ffl_loss = ffl(rgb, target_s)
            #if use focal loss, use MSE as well
            ...
            #Use L1 loss
            # img_loss = img2l1(rgb, target_s)
            # loss = loss + img_loss
            # psnr = l12psnr(img_loss)
            # trans = extras['raw'][...,-1]
            ...
            if USE_GAN and WGAN: #GAN loss, at the same iteration with consistency loss
                if USE_RANDOM_RAYS:
                    optimizer_d.zero_grad()
                    real_scores_d = dnet(target_s.permute(1,0))
                    fake_scores_d = dnet(rgb.permute(1,0))
                    real_loss = torch.mean(real_scores_d, dim=0)
                    fake_loss = torch.mean(fake_scores_d, dim=0)
                    d_loss = fake_loss - real_loss
                    d_loss.backward()
                    optimizer_d.step() # Update Critic

                    #Clip the weights in Discriminator
                    for p in dnet.parameters(): # might need further tuning
                        p.data.clamp_(-0.01, 0.01)
                    preds = dnet(rgb.permute(1,0))
                    loss = loss - 0.1*torch.mean(preds) # Update Generator
            else:
                optimizer_d.zero_grad()
                real_preds_d = dnet(target_s.permute(1,0))
                fake_preds_d = dnet(rgb.detach().permute(1,0))
                real_loss = criterion(real_preds_d, torch.ones_like(real_preds_d))
                fake_loss = criterion(fake_preds_d, torch.zeros_like(fake_preds_d))
                d_loss = (real_loss + fake_loss) / 2
                d_loss.backward()
                optimizer_d.step() # Update Discriminator
                preds = dnet(rgb.permute(1,0))
                gan_loss = criterion(preds, torch.ones_like(preds))
                loss = loss + 0.01*gan_loss # Update Generator
